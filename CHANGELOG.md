...
### Changed
- `depwolf remediate --format table` no longer shows an Applicable column in
  the overview table; applicability is still computed and available in the
  JSON output.
...
### Fixed
- Version-range applicability is enforced again (YES/NO/UNKNOWN). A dependency
  with a known installed version is only reported when it falls inside the
  affected range; e.g. WebGoat's jquery 4.0.0 no longer produces actionable
  findings (Version "-" + Confidence exact) for CVE-2020-11022/CVE-2020-11023,
  which apply to jQuery before 3.5.0. Unknown versions are dropped with a safe
  "could not be determined" reason instead of being treated as confirmed.
- The scan-resolved installed version is propagated through remediation
  (`depwolf remediate <CVE>` reuses the same version, and `applicable` agrees
  with the scan), and versionless duplicate stack entries no longer shadow the
  resolved version from a lockfile.

## [0.1.6.2] - 2026-08-13

### Changed
- Restored the 0.1.6 codebase: the 0.1.6.1 provenance/tri-state release and the
  subsequent accuracy-hardening work were reverted on `main`; this patch release
  ships the reverted tree unchanged so PyPI matches the `v0.1.6` tag again.
  The `v0.1.6.1` release remains available for anyone on the interim version.
