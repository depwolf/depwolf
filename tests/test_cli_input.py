...
def test_scan_end_to_end_confirmed_finding_gates(tmp_path, monkeypatch, capsys):
    # Full scan pipeline on a trivy report: the InstalledVersion from the report
    # flows into the funnel (confirmed YES), stays in remediation, and the CI
    # gate fails (rc 1) because the finding is at/above the threshold.
    import depwolf.interfaces.cli as cli
    from tests.conftest import _build_file_index
    ...
    out, err = capsys.readouterr()
    assert "CVE-2021-44228" in out
    assert "2.14.1" in out
    assert "[gate]" in err
    assert "finding(s) at/above risk threshold" in err
    # The finding keeps its confirmed verdict and version through remediation.
    assert "needs_verification: 0" in out


def test_scan_unknown_version_is_verification_not_actionable(tmp_path, monkeypatch, capsys):
    # A report entry without InstalledVersion is a confirmed-unknown: the scan
    # must NOT produce an actionable finding (gate passes) but must surface it
    ...
    report.write_text(
        json.dumps(
            {"Results": [{"Vulnerabilities": [{"VulnerabilityID": "CVE-2021-44228", "PkgName": "log4j-core"}]}]}
        ),
        encoding="utf-8",
    )
    ...
    assert rc == 0
    out, _err = capsys.readouterr()
    assert "needs_verification: 1" in out
    assert "CVE-2021-44228" not in out  # not a prioritized/actionable finding
